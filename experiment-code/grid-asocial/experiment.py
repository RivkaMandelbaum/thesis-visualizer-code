...
class NecklaceNAFCPage(ModularPage):
    def __init__(
        ...
        if CONDITION == ASOCIAL_CONDITION:
            activate_submit_without_click = False
                ...
                frames=[
                    Frame(
                        self.create_necklace_array(
                            ...
                            interactive=False, # click to answer = not interactive -> change for asocial condition?
                        ),
...
class CustomTrial(GraphChainTrial):
    ...
    def show_trial(self, experiment, participant):
        ...
        conditional_prompt_1 = "Error. Please contact the experimenters."
        if CONDITION == SOCIAL_CONDITION:
            conditional_prompt_1 = "Click on the necklace which you like most."
        elif CONDITION == ASOCIAL_CONDITION:
            conditional_prompt_1 = "Look at the necklace. Click the necklace or 'next' when you are ready to move on."

        page_1 = NecklaceNAFCPage(
            label="choose",
            prompt=conditional_prompt_1,
            necklace_states=options,
            color_options=COLOR_OPTIONS,
            css=["@media print { #graphic-prompt-container { display:none !important}}"], # disable printing the necklaces
        )

        conditional_prompt_ending = "Error. Please contact the experimenters."
        if CONDITION == SOCIAL_CONDITION:
            conditional_prompt_ending = "chose"
        elif CONDITION == ASOCIAL_CONDITION:
            conditional_prompt_ending = "saw"

        page_2 = NecklaceInteractivePage(
            label="reproduce",
            prompt=("Recolor this necklace like the necklace you just %s." % conditional_prompt_ending),
...
class CustomTrialMaker(GraphChainTrialMaker):
    ...
    def __init__(
        ...
        *,
        ...
        network_structure = self.generate_grid(grid_dimension, wrap_boundaries)
        super().__init__(
            id_=id_,
    ...
    def generate_grid(self, size, wrap_boundaries):
        vertices = []
        edges = []
        for r in range(size):
            for c in range(size):
                ...
                # if in the asocial condition, edges should remain empty
                # if in social condition, must add edges here
                if CONDITION == SOCIAL_CONDITION:
                    if r > 0:
                        upper_neighbour = int(
                    ...
                    # add boundary wrapping to ensure that all nodes have the same number of neighbours
                    if r == 0 and wrap_boundaries:
                        upper_neighbour = int(
                            np.ravel_multi_index([size - 1, c], [size, size])
                        )
                        edges.append(
                            {
                                "origin": v,
                    ...
                    if c == size - 1 and wrap_boundaries:
                        right_neighbour = int(
                            np.ravel_multi_index([r, 0], [size, size])
                        )
                        edges.append(
                            {
                                "origin": v,
